...
python -m network.training -r $root -m $model -N $size -s $shifts -w $width -e $epochs -l $lr -b $beta1 $beta2 -B $batchsize -d $savedir -v --force
echo "Network has finished training"
echo "Beginning testing..."
python -m network.testing -r $root -m $model -N $size -s $shifts -w $width -B $batchsize -v -f $savedir/${model}_${epochs}.tar
