...
savedir="../network/models" # directory in which to save the models during training
model="base" # model type. must be one of 'window' or 'base'
...
python ../network/training.py -r $root -m $model -N $size -s $shifts -w $width -e $epochs -l $lr -b $beta1 $beta2 -B $batchsize -d $savedir -v
