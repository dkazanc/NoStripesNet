...
This document aims to explain how to train & test the models in this project, including a description of the possible parameters you can specify and how they affect a model's performance.<br>
...
This is because often you will have quite a few models stored at once, and storing them directly in `NoStripesNet/` or one of the pre-existing subdirectories can quickly clutter up the project.<br>

There are several types of model that can be trained:
...
Each mode will be covered in more detail later in the document.<br>
...
```
There is also a [bash script](../submit.sh) to train a model. This is SLURM-compatible, so can be ran on an HPC cluster, and is also configured to train the model across multiple nodes using multiple GPUs. Arguments must be specified inside the script.

The training script supports both training from scratch and pretrained models.<br>
The training script will run for the specified number of epochs, then once completed will save the trained model (as long as a save directory has been specified).<br>
...
The following options are specific to training:
  ...
  - `--force`
    ...
    - This will make all plots non-blocking, and will pass 'y' to all user inputs.
    - This option is used in `../run_scripts/train_test.sh` so that testing runs straight after training, as well as in `submit.sh`.
  - `--save-every`
    - Interval (in epochs) at which to save the model.
    - For example, if options `--epochs 100 --save-every 25` are given, the model will be saved after epochs 0, 25, 50, 75 and 100.
    - If not given, the model is only saved after the last epoch.
...
```
Alternatively, there is a [bash script](../run_scripts/train_test.sh) which combines training and testing into one script. However, while this is SLURM-compatible, it is not configured for multi-node use, so might be slower. Arguments must be specified within the script.
...
There are a number of possible testing metrics, each identified using a number:<br>
> 0\. L1 (or Mean Absolute Error)<br>
> 1\. L2<br>
> 2\. Mean Squared Error<br>
> 3\. Sum of the absolute difference between gradients of inputs and targets<br>
> 4\. Dice Coefficient (or F1 score)<br>
> 5\. Intersection over Union<br>
> 6\. Histogram Intersection<br>
> 7\. Structural Similarity<br>
> 8\. Peak Signal to Noise Ratio<br>
...
The Generator class for Patch models is `PatchUNet`, and the Discriminator class is `PatchDiscriminator`<br>
The GAN class is `MaskedGAN`.<br>
